jQuery(document).ready(function () {
    ...
    var grid = jQuery('.grid').imagesLoaded(function () {
        ...
        // Make overlay always visible and do not reload on mouseout
        jQuery('.portfolio-text-holder').show();
    });
...
jQuery(function() {
    ...
    jQuery('.carousel-prev').click(function(e) {
        e.preventDefault();
        showItem((currentIndex - 1 + totalImages) % totalImages);
    });
    
    // Next button
    jQuery('.carousel-next').click(function(e) {
        e.preventDefault();
        showItem((currentIndex + 1) % totalImages);
    });
    
    // Click on image to go to next
    jQuery('.carousel-item img').click(function(e) {
        e.preventDefault();
        showItem((currentIndex + 1) % totalImages);
    });

    // Protected Image Modal functionality
    var protectedPassword = "elainedesign"; // Set your password here
    var modal = jQuery('#protectedImageModal');
    var closeBtn = jQuery('.protected-modal-close');
    
    jQuery('.protected-image').click(function(e) {
        e.preventDefault();
        var imageSrc = jQuery(this).data('image');
        
        // Prompt for password
        var enteredPassword = prompt("Enter password to view this image:");
        
        if (enteredPassword === protectedPassword) {
            jQuery('#unblurredImage').attr('src', imageSrc);
            modal.show();
